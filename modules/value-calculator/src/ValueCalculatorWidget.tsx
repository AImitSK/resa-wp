/**
 * Value Calculator Widget — Orchestrates the full flow:
 *
...
import { ValueResult } from './result/ValueResult';
...
import type { ModuleConfig, ValueCalculationResult, ValueCalculatorData } from './types';

/**
 * Type guard to validate wizard data conforms to ValueCalculatorData.
 * Since all fields are optional, we just verify it's a valid object.
 */
function isValueCalculatorData(data: unknown): data is ValueCalculatorData {
	return typeof data === 'object' && data !== null;
}

type Phase = 'loading' | 'wizard' | 'calculating' | 'lead-form' | 'result' | 'error';

interface ValueCalculatorWidgetProps {
	/** Pre-selected city slug from shortcode attribute. */
	presetCity?: string;
}

export function ValueCalculatorWidget({ presetCity }: ValueCalculatorWidgetProps) {
	const [phase, setPhase] = useState<Phase>('loading');
	const [config, setConfig] = useState<ModuleConfig | null>(null);
	const [wizardData, setWizardData] = useState<ValueCalculatorData>({});
	...
	useEffect(() => {
		api.get<ModuleConfig>('modules/value-calculator/config')
			.then((cfg) => {
				setConfig(cfg);
				...
				setPhase('wizard');
				trackEvent('asset_view', 'value-calculator');
			})
			.catch(() => {
	...
	const steps: StepConfig[] = useMemo(() => {
		...
		allSteps.push({
			...
			component: (props: {
				...
				const formData = props.data as ValueCalculatorData;
				const cityBounds =
	...
	const handleWizardComplete = useCallback(async (data: WizardData) => {
		if (!isValueCalculatorData(data)) {
			setErrorMessage(__('Ungültige Formulardaten', 'resa'));
			setPhase('error');
			return;
		}
		// Cast to ValueCalculatorData after validation (Zod schemas validate the actual values).
		const formData = data as ValueCalculatorData;
		setWizardData(formData);
		setPhase('calculating');

		trackEvent('asset_start', 'value-calculator', {
			location_id: formData.city_id,
		});

		try {
			// Calculate.
			const calcResult = await api.post<ValueCalculationResult>(
			...
			trackEvent('form_view', 'value-calculator', {
				location_id: formData.city_id,
			});
	...
	const handleLeadSubmit = useCallback(
		async (formData: Record<string, unknown>) => {
			...
			try {
				...
				trackEvent('form_submit', 'value-calculator', {
					location_id: wizardData.city_id,
				});

				trackEvent('result_view', 'value-calculator', {
					location_id: wizardData.city_id,
				});
	...
	const renderContent = () => {
		...
		if (phase === 'result' && result) {
			return <ValueResult result={result} inputs={wizardData} />;
		}
