...
/**
 * REST API controller for the Immobilienwert-Rechner module.
 *
 * POST /modules/value-calculator/calculate — Public calculation
 * GET  /modules/value-calculator/config    — Module config (cities, steps)
 */
class ValueCalculatorController extends RestController {
	...
	public function registerRoutes(): void {
		register_rest_route(
			self::NAMESPACE,
			'/modules/value-calculator/calculate',
			[
				'methods'             => 'POST',
		...
		register_rest_route(
			self::NAMESPACE,
			'/modules/value-calculator/config',
			[
				'methods'             => 'GET',
	...
	/**
	 * POST /modules/value-calculator/calculate
	 *
	...
	public function calculate( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		$addressLat = isset( $params['address_lat'] ) ? (float) $params['address_lat'] : null;
		$addressLng = isset( $params['address_lng'] ) ? (float) $params['address_lng'] : null;
		...
		$result = ValueCalculatorService::calculate( $inputs, $locationData );
	...
	/**
	 * GET /modules/value-calculator/config
	 *
	...
	private function getCalculationData( int $locationId ): array {
		...
		if ( $moduleSettings ) {
			...
			if ( $locationValues ) {
				...
				if ( isset( $locationValues['sale_factor'] ) && $locationValues['sale_factor'] > 0 ) {
					$calculationData['sale_factor'] = $locationValues['sale_factor'];
				}
			}
		...
		// Fallback: Try rent-calculator settings (they share the same base data).
		$rentSettings = ModuleSettings::getBySlug( 'rent-calculator' );
		if ( $rentSettings && ! empty( $rentSettings['factors'] ) ) {
			$calculationData = $rentSettings['factors'];

			// Add default sale_factor if not set.
			if ( ! isset( $calculationData['sale_factor'] ) ) {
				$calculationData['sale_factor'] = 25;
			}

			// Apply location-specific overrides.
			$locationValues = $rentSettings['location_values'][ (string) $locationId ] ?? null;
			if ( $locationValues && isset( $locationValues['base_price'] ) && $locationValues['base_price'] > 0 ) {
				$calculationData['base_price'] = $locationValues['base_price'];
			}
		...
		if ( $location ) {
			...
			$presets    = ValueCalculatorService::getRegionPresets();
