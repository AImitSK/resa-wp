...
ISM nutzt ein **Hybrid-Modell**: Freemium-Kern (Free → Premium) plus einzeln kaufbare Add-ons für Integrationen. Dieses Modell ist optimal weil:
...
**Ziel:** Verbreitung, Sichtbarkeit auf WordPress.org, Vertrauen aufbauen. Der Makler soll ISM installieren, ausprobieren und den Wert erkennen — dann upgraden.
...
Automationen                     ❌
Shortcode-Generator              ✅
Branding entfernbar              ❌           "Powered by ISM" bleibt
Webhooks                         ❌
Integrationen                    ❌
...
| ISM-Branding | Kostenlose Werbung für ISM, Makler will eigenes Branding |
...
**Ziel:** Das vollständige ISM-Erlebnis. Alles was der Makler braucht, um professionell Leads zu generieren, zu verwalten und nachzufassen. Integrationen sind bewusst NICHT enthalten — die kommen als Add-ons.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PDF-System
  PDF-Ergebnis per E-Mail        ✅ Ohne ISM-Branding
...
Branding                         ✅ Eigenes Branding / ISM entfernbar
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Add-ons sind eigenständige Plugins die ISM Premium voraussetzen. Jedes Add-on wird separat über Freemius lizenziert und aktualisiert.

### Voraussetzung

```
⚠ Alle Add-ons erfordern eine aktive ISM Premium-Lizenz.
...
```
Name:           ISM für onOffice
Slug:           ism-onoffice
Voraussetzung:  ISM Premium (aktiv)
...
- Setzt konfigurierbare Tags (z.B. "ISM-Lead", "Vermieter", Stadtname)
- Optional: Aufgabe für zuständigen Makler erstellen
- Feldmapping konfigurierbar (ISM-Feld → onOffice-Feld)
...
```
Name:           ISM für CleverReach
Slug:           ism-cleverreach
Voraussetzung:  ISM Premium (aktiv)
...
```
Name:           ISM für Brevo
Slug:           ism-brevo-newsletter
Voraussetzung:  ISM Premium (aktiv)
Preis:          49 €/Jahr (1 Site)
```

**Hinweis:** Dies ist das Newsletter-Add-on für Brevo. Der Brevo-SMTP-Relay für den Transaktions-E-Mail-Versand ist bereits in ISM Premium enthalten. Das Add-on ergänzt die Newsletter-Funktionalität (Listen, Tags, Automationen).
...
```
Produkt:        ISM — Immobilien Smart Assets
...
Requires: ISM Premium (active license check via Freemius API)
```

### SDK-Integration (PHP-Seitig)

```php
// Haupt-Plugin: ism.php
if ( ! function_exists( 'ism_fs' ) ) {
    function ism_fs() {
        global $ism_fs;
        if ( ! isset( $ism_fs ) ) {
            require_once dirname( __FILE__ ) . '/freemius/start.php';
            $ism_fs = fs_dynamic_init( array(
...
if ( ism_fs()->is_plan( 'premium' ) ) {
    // Premium-Features laden
}

// Prüfung: Ist ein bestimmtes Add-on aktiv?
if ( ism_fs()->is_addon_activated( 'ism-onoffice' ) ) {
    // onOffice-Integration laden
}

// Prüfung: Free-Limits
function ism_can_add_location() {
    if ( ism_fs()->is_plan( 'premium' ) ) return true;
    $count = ism_get_location_count();
    return $count < 1; // Free: max 1 Location
}

function ism_can_activate_asset( $asset_slug ) {
    if ( ism_fs()->is_plan( 'premium' ) ) return true;
...
An strategischen Stellen im Free-Plugin zeigt ISM Upgrade-Hinweise:

```
┌─────────────────────────────────────────────────┐
│  ⭐ Immobilienwert-Kalkulator                   │
│                                                 │
│  Dieser Smart Asset ist Teil von ISM Premium.   │
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ISM Premium           Lead-Magneten 149 €         18 Assets
ISM + CRM-AddOn       + Integration 228 €         18 + CRM-Anbindung
ISM Profi-Bundle      All-in        249 €         18 + CRM + Newsletter
```

**Positionierung:** ISM ist nicht ein einzelner Rechner, sondern eine **Toolbox mit 18 Lead-Magneten** für einen Bruchteil des Preises einer einzelnen Sprengnetter-Lizenz. Plus eigene Daten, eigenes Branding, eigene Leads.
